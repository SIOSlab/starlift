...
orbitName = 'TrajI_1265_MassOptimal/'
...
#    with open(fileDirectory+orbitName+'vary'+fileName+'.txt', 'r') as ff:
        lines = ff.readlines()
...
#        plt.title(thrusterName)
        ...
        plt.figure(3, figsize=(20, 8))
        plt.plot(Isps_plt.value, t_burns_plt/60/60, lineStyles[jj], label=labels[jj])
        plt.xlabel('Specific Impulse [s]')
#        plt.ylabel('Total Burn Time [s]')
        plt.ylabel('Total Burn Time [hr]')
#        plt.title(thrusterName)
        plt.legend(loc='upper left', bbox_to_anchor=(.01, 1.5))
        
        plt.subplots_adjust(left=0.12, right=0.95, top=0.7, bottom=0.11)
        plt.savefig(fileDirectory+orbitName+'ispVsBurnTime.png')

        plt.figure(4, figsize=(20, 8))
        plt.plot(Isps_plt.value, pos_errors_plt.value*1000, lineStyles[jj], label=labels[jj])
        plt.xlabel('Specific Impulse [s]')
#        plt.ylabel('Max Position Error [km]')
