...
gmat_timecan_dt = np.zeros(len(gmat_time))
...
for ii in np.arange(len(gmat_time)):
    ...
    gmat_timecan_dt[ii] = unitConversion.convertTime_to_canonical(gmat_time[ii].value*u.d) - unitConversion.convertTime_to_canonical(gmat_time[0].value*u.d)

# Convert back to R frame, to check...
for ii in np.arange(len(gmat_time)):
    gmat_posrot2[ii, :] = frameConversion.inert2rotP(gmat_posinert[ii, :], gmat_time[ii], gmat_time[0])

np.set_printoptions(threshold=sys.maxsize)
print(gmat_timecan_dt)
breakpoint()
