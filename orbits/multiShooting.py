...
def statePropFFI_ECI(Ts,state0,GM):
    ...
    sol_int = solve_ivp(ffInertial_ECI, [ti, tf], state0, args=(GM,), rtol=1E-12, atol=1E-12, method='LSODA')
...
def ffInertial_ECI(tt, w, GM, radii=None, uT=None, times=None):
    ...
    r_Moon = spice.spkpos('Moon', tt, 'J2000', 'None', 'Earth')[0]
    r_Earth = spice.spkpos('Earth', tt, 'J2000', 'None', 'Earth')[0]
    r_Sun = spice.spkpos('Sun', tt, 'J2000', 'None', 'Earth')[0]

    r_bodies = np.vstack((r_sc, np.vstack((-r_Moon, -r_Sun))))
    
    f_Earth = -GM[0]*(r_sc - r_Earth)/np.linalg.norm(r_sc - r_Earth)**3
    f_Moon = -GM[1]*(r_sc - r_Moon)/np.linalg.norm(r_sc - r_Moon)**3 - GM[1]*r_Moon/np.linalg.norm(r_Moon)**3
    f_Sun = -GM[2]*(r_sc - r_Sun)/np.linalg.norm(r_sc - r_Sun)**3 - GM[2]*r_Sun/np.linalg.norm(r_Sun)**3
...
def positionShooting_ECI(initialEpoch, initialState, targetEpoch, targetState, positionTolerance, GM):
    ...
    while np.linalg.norm(deltaR) > positionTolerance:
        # calculate state transition matrix
        state0 = np.append(initialState, phi0)
        times, states = statePropFFI_ECI(np.array([initialEpoch, targetEpoch]), state0, GM)
...
def multipleShootingI_ECI(initialEpoches, initialStates, positionTolerance, velocityTolerance, GM):
    ...
    while deltaV > velocityTolerance:
        ...
        print('Inner loop: position shooting')
        for ii in np.arange(N-1):
            cInitial, cFinal, STM, exitFlag1 = positionShooting_ECI(correctedInitialEpoches[ii], correctedInitialStates[ii,:], correctedInitialEpoches[ii+1], correctedInitialStates[ii+1,:], positionTolerance, GM)
            correctedInitialStates[ii,:] = cInitial
            correctedFinalStates[ii,:] = cFinal
            STMs[ii,:,:] = STM
            exitFlagLevel1[ii] = exitFlag1
        ...
        for ii in np.arange(1,N-1):
            ...
            a2minus = ffInertial_ECI(correctedInitialEpoches[ii], correctedFinalStates[ii-1, :], GM)
            a2minus = a2minus[3:6]
            a2plus  = ffInertial_ECI(correctedInitialEpoches[ii], correctedInitialStates[ii, :], GM)
            a2plus = a2plus[3:6]
