...
GM = np.array([gmEarth, gmMoon, gmSun])
...
for kk in np.arange(1,len(Tp_target)):
    ...
    mainDir = '/Users/gracegenszler/Documents/Research/starlift/orbits/graveyardOrbits/L2S_ECI/' + str(timeDays) + '_days/'
    ...
    while not giveUp:
        ...
        if crtbpFilePath.is_file():
            ...
            posCRTBP_R_dim = unitConversion.convertPos_to_dim(posCRTBP_R - np.array([-mu_star, 0, 0])).to_value(u.km)
            ...
            for ii in np.arange(len(etCRTBP_mjd)):
                Crv_R2I = spice.sxform('ECR','ECI',etCRTBP_mjd[ii])
...
#            velocityTolerance = 0.0001*orbs  # km/s
            ...
            while exitflag != 1:
                ...
                Crv_R2I = spice.sxform('ECR','ECI',initialEphmerisEpoches)
                initialEphemerisMCI = np.zeros((len(times_dim), 6))
                for ii in np.arange(len(times_dim)):
                    initialEphemerisMCI[ii,:] = Crv_R2I[ii,:,:] @ posvel[ii]

                correctedInitialEpoches, correctedInitialStates, exitflag = ms.multipleShootingI_ECI(initialEphmerisEpoches, initialEphemerisMCI, positionTolerance, velocityTolerance, GM)
        ...
        #            flag100Years = False
            ...
            if exitflag == 1:
                ...
                for ii in np.arange(N1-1):
                    ...
                    times, states = ms.statePropFFI_ECI(Ts, correctedInitialStates[ii,:], GM)
                        
                    inertialStates = np.vstack((inertialStates, states))
                    
                    Crv_I2R = spice.sxform('ECI','ECR',times)
        ...
        if os.path.exists(periluneDataFilePath):
            ...
            while not minPatch:
                ...
                correctedInitialEpoches, correctedInitialStates, exitflag = ms.multipleShootingI_ECI(initialEphmerisEpoches, posvel, positionTolerance, velocityTolerance, GM)
        ...
        if not oldFile:
            ...
            while not solutionFound:
                ...
                sol_int = solve_ivp(ms.ffInertial_ECI, [t100_initial, t100_final], state100_initial, args=(GM,radii), events=[ms.hitMoon, ms.hitEarth, ms.hitSun, ms.lostShape], rtol=1E-12, atol=1E-12, method='LSODA')
                time100 = sol_int.t
                states100 = sol_int.y.T
                time100Events = sol_int.t_events
                states100Events = sol_int.y_events
                
                Crv_I2R = spice.sxform('ECI','ECR',time100)
