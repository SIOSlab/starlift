function [E]=evalAtomsThrust(Ro)
% [E]=evalAtoms(ri)

%Calculate E = Objective Function
    %minimize Energy of System. We want min thrust
    %J(ri, mstruct,Isp) = 
...
ri = Ro(1); %[m] Initial Orbital Radius
mstruct = Ro(2); %[kg] Initial Spacecraft Structure Mass (without propellant)
Isp = Ro(3); %[s] Specific Impulse for low thrust continuous orbit raise
